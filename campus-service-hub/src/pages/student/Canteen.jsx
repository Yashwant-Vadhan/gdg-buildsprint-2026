...
import { Clock, AlertCircle, RefreshCw, Plus, Minus, Search, ShoppingCart, Eye } from 'lucide-react';
...
export default function Canteen() {
  ...
  // Multi-variant state for Ice Cream: mapping item.id -> selected variant price
  const [iceCreamPrices, setIceCreamPrices] = useState({});

  // Canteen Local Cart State: mapping item.id -> { item, qty, selectedPrice }
  ...
  const fetchItems = async () => {
    try {
      if (isMockMode) {
        // Enforce the new AU menu list by checking localStorage status
        const stored = localStorage.getItem('canteen_items');
        let parsed = stored ? JSON.parse(stored) : [];
        
        // Overwrite if stored items do not have Veg Puff or have old categories
        ...
        // Seed default Ice Cream variant price selector
  ...
  const addToCart = (item) => {
    setCart(prev => {
      ...
      // If it's Ice Cream, retrieve chosen variant price from local state
  ...
  // Checkout redirect passing cart items
  ...
  return (
    <div className="space-y-6 text-left pb-24 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-3 bg-white dark:bg-slate-900 border border-slate-205 dark:border-slate-800 rounded-xl px-4 py-2 shadow-sm text-xs font-bold text-slate-800 dark:text-slate-100">
      ...
      <div className="flex flex-col gap-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-805 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-brand transition-colors text-xs"
          />
        </div>
              ...
              >
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div className="text-left space-y-2">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded border shrink-0 ${
                        isOutOfStock
                          ...
                          ? 'bg-amber-50 dark:bg-amber-950/30 border-amber-250 dark:border-amber-900 text-amber-800 dark:text-amber-400'
                    ...
                    {/* Variant Selector for Ice Cream (₹20 / ₹25 / ₹30) */}
                    {item.name === 'Ice Cream' ? (
                          ...
                          {[basePrice, basePrice + 5, basePrice + 10].map((vPrice) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                // Update cart price if already in cart
                                if (cart[item.id]) {
                                  setCart(prevCart => ({
                                    ...prevCart,
                      ...
                      <p className="text-xs text-slate-450 dark:text-slate-400">Fresh and crispy, served hot from refectory stove.</p>
                    )}
                  </div>
                    ...
                    {cartItem ? (
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className="w-8 h-8 rounded-lg bg-white dark:bg-slate-855 border border-slate-250 dark:border-slate-700 text-slate-700 dark:text-slate-300 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="w-8 h-8 rounded-lg bg-white dark:bg-slate-855 border border-slate-250 dark:border-slate-700 text-slate-700 dark:text-slate-300 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-30 cursor-pointer"
                        >
      ...
      {/* Sticky Bottom Cart Bar on Mobile when items are added */}
      {cartTotalQty > 0 && (
        <div className="fixed bottom-14 md:bottom-4 left-4 right-4 md:left-auto md:right-8 md:w-96 bg-[#202124] text-white py-3 px-5 rounded-xl shadow-xl flex items-center justify-between z-40 animate-fadeIn border border-neutral-800">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-lg bg-[#e4002b] flex items-center justify-center text-white shrink-0">
          ...
          <button
            ...
            className="py-2 px-4 bg-[#e4002b] hover:bg-red-700 text-white rounded-lg text-xs font-black uppercase tracking-wider cursor-pointer transition-colors"
          >
            Checkout
          </button>
        </div>
