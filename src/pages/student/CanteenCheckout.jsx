...
export default function CanteenCheckout() {
  ...
  if (!item && cart.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 text-center max-w-md mx-auto mt-12 rounded-2xl shadow-sm">
        <p className="text-slate-800 dark:text-slate-200 font-bold mb-4">Your basket is empty</p>
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 text-left pb-10">
      ...
      <h1 className="text-xl font-black text-slate-800 dark:text-slate-100 tracking-tight uppercase m-0">Confirm Checkout</h1>

      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-6 shadow-md relative overflow-hidden">
        
        {/* Checkout List */}
        <div className="space-y-4">
          <p className="text-[10px] font-black uppercase text-slate-450 dark:text-slate-500 tracking-widest border-b border-slate-105 dark:border-slate-800 pb-2">Order summary</p>
          ...
          {hasCart ? (
              ...
              {cart.map((cItem) => (
                <div key={cItem.item.id} className="flex justify-between items-center text-xs font-semibold">
                  <div className="flex gap-2.5 items-center">
                    ...
                    <span className="text-slate-850 dark:text-slate-200 font-bold">{cItem.item.name}</span>
                  </div>
                  <span className="text-slate-800 dark:text-slate-100 font-bold">₹{(cItem.selectedPrice * cItem.qty).toFixed(2)}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="flex justify-between items-center text-xs font-semibold">
              <div className="flex gap-2.5 items-center">
                ...
                <span className="text-slate-850 dark:text-slate-200 font-bold">{item.name}</span>
              </div>
              <span className="text-slate-800 dark:text-slate-100 font-bold">₹{totalAmount.toFixed(2)}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-2 border-t border-slate-100 dark:border-slate-800 pt-4">
          ...
          <div className="flex justify-between items-center text-xs text-slate-800 dark:text-slate-100 pt-3 border-t border-slate-100 dark:border-slate-800 font-black uppercase tracking-wider">
        ...
        {/* Security badges */}
        <div className="flex justify-center items-center gap-4 text-[9px] text-neutral-400 pt-3 border-t border-neutral-100 dark:border-slate-800 font-black uppercase tracking-widest">
          <span className="flex items-center gap-1">
            <ShieldCheck className="w-3.5 h-3.5 text-green-700" /> Sandbox UPI Gateway
          </span>
        </div>
      </div>
    </div>
