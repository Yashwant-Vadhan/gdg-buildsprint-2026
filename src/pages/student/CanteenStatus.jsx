...
export default function CanteenStatus() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-neutral-800 bg-neutral-100 dark:bg-slate-950">
  ...
  if (error || !order) {
    return (
      <div className="bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-800 p-8 text-center max-w-md mx-auto mt-12 space-y-4 rounded-2xl shadow-md">
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 text-left pb-10">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-black text-neutral-800 dark:text-slate-105 tracking-tight uppercase m-0">Token Tracker</h1>
        <button 
          onClick={fetchOrder} 
          className="p-2 bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-800 rounded-xl hover:border-neutral-350 dark:hover:border-slate-700 text-neutral-500 cursor-pointer"
        >
      ...
      {/* Main Status Panel */}
      <div className="bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-800 rounded-2xl p-6 space-y-6 shadow-md text-center">
        
        {/* Token Card */}
        <div>
          <span className="text-[10px] text-neutral-450 uppercase font-black tracking-wider">Your Order Token</span>
          <p className="text-5xl font-black text-slate-800 dark:text-slate-100 mt-1 tracking-widest">#{order.token_no}</p>
        </div>
        ...
        <div className={`p-4 rounded-xl border flex items-center justify-center gap-2 text-xs font-black uppercase tracking-wide ${
          order.status === 'Collected'
            ? 'bg-neutral-50 dark:bg-slate-800 border-neutral-200 dark:border-slate-700 text-neutral-500'
            : order.status === 'Ready'
            ? 'bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800 text-green-700 dark:text-green-400'
            : 'bg-red-50 dark:bg-red-955/30 border-red-200 dark:border-red-900 text-[#e4002b]'
        }`}>
          {order.status === 'Collected' ? (
        ...
        {order.status !== 'Collected' && (
          <div className="bg-neutral-50 dark:bg-slate-800 border border-neutral-200 dark:border-slate-700 rounded-xl p-3 text-xs text-neutral-500 dark:text-slate-400 flex items-center gap-2 font-semibold">
            ...
            return (
              <div key={step} className="flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all ${
                  isActive 
                    ? 'border-[#e4002b] bg-red-50 dark:bg-red-950/30 text-[#e4002b] font-black scale-105 shadow'
                    : isCompleted 
                    ? 'border-green-600 bg-green-50 dark:bg-green-950/30 text-green-700'
                    : 'border-neutral-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-neutral-400 font-bold'
                }`}>
                ...
                <span className={`text-[8px] font-black mt-2 uppercase tracking-widest ${
                  isActive ? 'text-[#e4002b]' : isCompleted ? 'text-neutral-550' : 'text-neutral-400'
                }`}>
                  {step}
                </span>
              </div>
        ...
        {order.status !== 'Collected' ? (
          <div className="flex flex-col items-center space-y-4 border-t border-neutral-100 dark:border-slate-800 pt-6">
            <div>
              <p className="text-xs font-black text-neutral-800 dark:text-slate-205 uppercase tracking-tight">Present Ticket QR Code</p>
          ...
          <div className="border-t border-neutral-100 dark:border-slate-800 pt-6 text-neutral-400 text-xs font-bold uppercase tracking-wider">
        ...
        {isMockMode && order.status !== 'Collected' && (
          <div className="border-t border-neutral-150 dark:border-slate-800 pt-4 mt-2">
